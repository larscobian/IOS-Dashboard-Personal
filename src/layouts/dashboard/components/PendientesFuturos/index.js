...
const PendientesFuturos = () => {
  ...
  const [pendientes, setPendientes] = useState(() => {
    const saved = localStorage.getItem('pendientesFuturos');
  ...
  const agregarPendiente = () => {
    if (nuevoPendiente.trim()) {
      ...
      localStorage.setItem('pendientesFuturos', JSON.stringify(nuevosPendientes));
      setNuevoPendiente('');
    }
  };
  ...
  return (
    <Card sx={{ height: '340px', overflow: 'hidden' }}>
      <VuiBox display='flex' flexDirection='column' height="100%">
        ...
        <VuiTypography variant='button' color='info' fontWeight='regular' mb='12px'>
        ...
        <VuiBox display="flex" gap="8px" mb={2}>
          <TextField
            fullWidth
            size="small"
            placeholder="Nueva tarea para el futuro..."
...
export default PendientesFuturos;
